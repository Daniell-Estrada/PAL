...
public class CertificateController {
  ...
  /**
   * Endpoint para generar un certificado de curso para un estudiante. Solo los estudiantes que
   * hayan completado el curso pueden generar el certificado.
   */
  @PostMapping("/generate/{courseId}")
  public ResponseEntity<String> generateCertificate(
      @RequestParam Long userId, @PathVariable Long courseId) {
    try {
  ...
  @PostMapping("/send-email/{certificateId}")
  public ResponseEntity<String> sendCertificateEmail(
      @PathVariable Long certificateId, @RequestParam String email) {
    try {
      certificateService.sendCertificateEmail(certificateId, email);
      return ResponseEntity.ok("Certificado enviado por correo.");
    } catch (Exception e) {
      return ResponseEntity.status(500).body("Error al enviar el certificado: " + e.getMessage());
    }
  }
  ...
  @GetMapping("/my-certificates/{studentId}")
  public ResponseEntity<?> getMyCertificates(@PathVariable Long studentId) {
